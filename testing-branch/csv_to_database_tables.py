...
# Read the CSV file
csv_file = 'resources/CSE A-3.csv'
# Get the absolute path
csv_file_path = os.path.abspath(csv_file)

def studentTableCreation(csv_file, csv_file_path):
    ...
    sem_df = pd.read_csv(csv_file_path, skiprows = 1, usecols = ["Sem"])
    ...
    table_name_extra = re.findall('/.*\.',csv_file)
    table_name = table_name_extra[0][1:-1:]
...
def facultyTableInsertion(csv_file_path):
    # Read the CSV file, skipping initial rows and using row 3 as header
    df1 = pd.read_csv(csv_file_path, skiprows = 3, usecols = ['Sr. No', 'USN','Students Name'])
...
studentTableCreation(csv_file, csv_file_path)

facultyTableInsertion(csv_file_path)